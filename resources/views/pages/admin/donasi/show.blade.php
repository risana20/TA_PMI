...
<div id="modal-map-stock" class="hidden fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-5 sm:p-6">
        ...
        <form action="{{ route(request()->segment(1) . '.' . 'donasi.complete', $donasi) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <select name="stok_logistik_id" required class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 appearance-none bg-white">
                    ...
                    @foreach($stokLogistik as $stok)
                        <option value="{{ $stok->id }}" data-satuan="{{ $stok->itemLogistik->satuan }}">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Jumlah/Bobot Real yang Diterima</label>
                <div class="relative flex items-center">
                    <input type="number" name="jumlah" id="map-stock-jumlah" required min="1" 
                        value="{{ $donasi->jenis === 'Barang' ? ($donasi->pemasukanLogistik->jumlah ?? 1) : ((int) filter_var($donasi->donasiMakanan->jumlah_makanan ?? '1', FILTER_SANITIZE_NUMBER_INT) ?: 1) }}"
                        class="w-full border border-gray-200 rounded-lg pl-3 pr-16 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <span id="map-stock-satuan" class="absolute right-3 text-xs text-gray-400 font-semibold pointer-events-none"></span>
                </div>
                <p class="text-xs text-gray-500 mt-1">Masukkan jumlah/bobot real yang akan ditambahkan ke stok logistik.</p>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    const selectEl = document.querySelector('select[name="stok_logistik_id"]');
    const satuanEl = document.getElementById('map-stock-satuan');
    
    if (selectEl && satuanEl) {
        const updateSatuan = () => {
        ...
        selectEl.addEventListener('change', updateSatuan);
        updateSatuan();
    }
});
</script>
